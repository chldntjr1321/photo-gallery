...
목표: `useState`와 `useEffect`의 의존성 배열을 이해하며 검색/필터 흐름을 구현한다.

### 할 일

- [x] 검색어 상태(`query`) 추가
- [x] 검색창 `SearchBar` 컴포넌트 작성
- [x] `query`가 바뀔 때마다 API를 다시 호출하도록 `useEffect` 의존성 배열 조정
- [x] 검색어가 없을 때는 기본 피드(`/photos/random`) 표시
- [x] 검색 결과가 없을 때 "결과 없음" 메시지 표시
- [x] 방향 필터 상태(`orientation`) 추가 — `landscape` / `portrait` / `all`
- [x] 가로/세로 버튼 클릭 시 `orientation` 상태 변경
- [x] `orientation`이 바뀔 때도 API 재호출되도록 의존성 배열에 추가

### 체크포인트

- 검색창에 단어를 입력하면 관련 이미지로 결과가 바뀐다
- 검색어를 지우면 기본 피드로 돌아온다
- 가로/세로 버튼을 누르면 해당 방향 사진으로 결과가 바뀐다
